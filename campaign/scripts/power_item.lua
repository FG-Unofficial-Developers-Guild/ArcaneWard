...
function optionChange(bClose)
	...
	if
		not bCallbacksRegistered and
			(OptionsManager.isOption("ARCANE_WARD_SPELL_CAST_GAME", "on") or
				OptionsManager.isOption("ARCANE_WARD_SPELL_CAST", "on") or
				aCastInfo.bHasArcaneWard)
	 then
		registerCallbacks(aCastInfo)
		bCallbacksRegistered = true
	elseif
		(not aCastInfo.bHasArcaneWard and bCallbacksRegistered and
			OptionsManager.isOption("ARCANE_WARD_SPELL_CAST_GAME", "off") and
			OptionsManager.isOption("ARCANE_WARD_SPELL_CAST", "off")) or
			(bClose and bCallbacksRegistered)
	 then
...
function registerCallbacks(aCastInfo)
	...
	if aCastInfo.bSpellcasting then
		if aCastInfo.bUpcast then
			...
			DB.addHandler(
				sNodePath .. ".powermeta.spellslots" .. tostring(aCastInfo.nLevel) .. ".used",
				"onUpdate",
				onDisplayChanged
			)
		end
...
function onDisplayChanged()
	...
	elseif
		sDisplayMode == "action" and bProcess and (aCastInfo.bSpellcasting or aCastInfo.bPactMagic) and aCastInfo.nLevel > 0
	 then
...
function setCastButton(aCastInfo)
	if
		aCastInfo.bHasArcaneWard or OptionsManager.isOption("ARCANE_WARD_SPELL_CAST_GAME", "on") or
			OptionsManager.isOption("ARCANE_WARD_SPELL_CAST", "on")
	 then
